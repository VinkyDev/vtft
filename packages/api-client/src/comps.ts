...
export async function queryComps(params?: CompQueryParams): Promise<CompResponse> {
  ...
  const response = await getApiClient().get<CompResponse>('/comps', { params: queryParams })
...
export async function getCompDetails(compId: string): Promise<CompDetailsResponse> {
  const response = await getApiClient().get<CompDetailsResponse>(`/comps/${compId}?includeDetails=true`)
