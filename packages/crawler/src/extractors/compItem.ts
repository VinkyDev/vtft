...
const logger = new Logger({ namespace: 'crawler', scope: 'xtr/compItem' })
...
export async function extractRecommendedItems(
  ...
  try {
        ...
        if (caption && caption.includes('道具')) {
          ...
          for (const row of rows) {
            try {
              ...
              logger.error('提取道具行失败', error as Error)
            }
          }
        }
      }
      catch (error) {
        logger.error('处理道具表格失败', error as Error)
      }
    }
    ...
    logger.error('提取推荐道具列表失败', error as Error)
  }
